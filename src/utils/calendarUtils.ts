...
export const createOrUpdateEmployeeCalendar = async (
  ...
  try {
    // Busca o padrão de horários existente
    ...
    if (!calendarDoc.exists()) {
      // Se o calendário não existe, cria um novo e aplica o padrão de horários
      const newCalendar = generateCalendarFor15Days();
      const calendarioComPadrao = applySchedulePattern(
        newCalendar,
        schedulePattern
      );
      ...
      if (!calendarData.calendar || calendarData.calendar.length < 15) {
        // Se o calendário estiver vazio ou tiver menos de 15 dias, cria um novo
        const newCalendar = generateCalendarFor15Days();
        const calendarioComPadrao = applySchedulePattern(
          newCalendar,
          schedulePattern
...
export const deleteEmployeeCalendar = async (
  ...
  try {
    await deleteDoc(doc(db, "calendar", employeeName));
  } catch (error) {
